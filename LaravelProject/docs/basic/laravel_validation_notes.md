...
- **Form Request** 是 Laravel 專為表單驗證與授權封裝的請求類別，繼承自 `Illuminate\Foundation\Http\FormRequest`。
...
3. *Controller 型別提示該 Request*
   - 只要在 Controller 方法參數型別提示 Form Request，Laravel 會自動注入並執行驗證/授權：
   ```php
...
4. *自動驗證與授權*
   ...
   - 驗證失敗自動重導回前頁，錯誤訊息自動帶入 session
   - 授權失敗自動回傳 403，Controller 不會執行
...
> - 此方法用來定義「每個欄位的驗證規則」。
> - 回傳一個陣列，key 為欄位名稱，value 為規則字串或陣列。
...
// 適合 Controller 內快速驗證，XHR/JSON 請求失敗時自動回傳 422 JSON
// validateWithBag('post')：指定錯誤訊息存入特定 error bag，適合多表單同頁時分開顯示錯誤
...
### *自訂錯誤訊息（多種方式）*
```php
// Validator::make 第三個參數可直接傳自訂訊息陣列
...
### *自訂屬性名稱與 values*
```php
// Validator::make 第四個參數可自訂欄位名稱顯示，錯誤訊息會自動帶入
...
// Blade 內的 $errors 變數也是 MessageBag
...
// custom 區塊可針對欄位+規則自訂訊息，attributes 區塊自訂欄位名稱，values 區塊自訂規則值顯示
...
// 這種設計讓錯誤訊息更貼近用戶語言，避免顯示技術縮寫或代碼，適合用於選單、狀態、類型等欄位的友善顯示
```
...
- **unique:table,column**
  ...
  - 範例：
    ...
    ```
  - *註：* ignore 只能用系統產生的唯一值，避免 SQL injection
...
- **timezone**
  ...
  - 可指定範圍：
    ...
    'timezone' => 'required|timezone:per_country,US'
    ```
  - *註：* 支援 PHP DateTimeZone::listIdentifiers 的所有參數
...
- **Validator::sometimes 方法**
  ...
  - 範例：
    ...
    ```
  - *註：* $input 會是 Fluent 實例，可存取所有輸入資料
...
- **File::types / min / max / image / dimensions**
  ...
  - 範例：
    ...
    ```
  - *註：* image 預設不允許 SVG，需 image:allow_svg 或 File::image(allowSvg: true)
