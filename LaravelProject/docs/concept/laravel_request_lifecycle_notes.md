...
所有請求進入點為 `public/index.php`，由 web server（Apache/Nginx）導向此檔。
...
*補充註解*：
**Service Container（服務容器）的類別**是 `Illuminate\Container\Container`，
而 **Application 的類別**是 `Illuminate\Foundation\Application`。
