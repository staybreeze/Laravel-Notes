...
- 可在 **credentials 陣列**加上其他條件（如 active 狀態）：
  ```php
...
### 8.3 *已登入自動導向*
- 用 `guest` middleware 保護註冊/登入頁，**已登入者** 自動導向 dashboard。
...
### 9.1 *sanctum（推薦）*
- **Sanctum 是什麼？**
  - Sanctum 是 Laravel 官方推出的 *輕量級 API 認證套件*，適合 SPA（單頁應用）、行動 App、第一方 Web + API 等情境。
...
- 不產生 session/cookie，適合 **API**：
  ...
  - **驗證失敗**會自動回傳 401，瀏覽器跳出帳密輸入框
  - **驗證成功**才會繼續往下走（但不會登入，只針對這次請求有效）
  ...
  - **驗證成功**時回傳 null，繼續執行 $next($request)
  - *為什麼驗證成功要回傳 null？*
  ...
  - *補充：?: 和三元運算子的差異*
    ...
    - 例：
        ...
        ```
    - `?: 是 PHP 的 null 合併運算子（又稱 Elvis operator）`，
      等同於 if-else 判斷：
      ```php
      return A ?: B; 等同於 if (A) { return A; } else { return B; }
      ```
    - 常見用法：
      - 當左邊運算式為 **null/false/空字串**時，會執行右邊的運算式
